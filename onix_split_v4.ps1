...
Get-ChildItem -Path $path -Filter *xml | ForEach-Object {
    ...
    $fileid = 0
    $sep = "_"
    $subsections = "descriptivedetail", "collateraldetail", "publishingdetail", "productsupply", "relatedmaterial"
    ...
    foreach ($p in $allproducts) {
        ...
        foreach ($section in $subsections) {
            ...
            $cnode = $p.SelectSingleNode($section)

            if ($cnode) {
                $newfilename = [IO.Path]::Combine($fileNameDirectory, $unqId + $sep + $section + $fileNameExt)
                if (Test-Path $newfilename) {
                    $newfilename = [IO.Path]::Combine($fileNameDirectory, $unqId + $sep + $section + $sep + $fileid + $fileNameExt)
                }
            
                $newFile = [xml]($cnode.OuterXml)
                $newFile.Save($newfilename)

                $p.RemoveChild($cnode) | Out-Null
            }
        }
